...
import (
	...
	"github.com/lomik/prometheus_png"
)
...
func main() {
	...
	handler, err := prometheus_png.NewPNG(config.Main.PrometheusAddr, config.Main.PrometheusPath, config.Main.PrometheusAuth, config.Main.CACert, config.Main.Timeout)
	if err != nil {
		log.Fatal(err)
	}
	http.Handle("/", handler)
